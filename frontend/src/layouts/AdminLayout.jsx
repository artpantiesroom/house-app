...
const links = [
  { to: '/admin/dashboard', label: 'Панель', icon: 'dashboard' },
  { to: '/admin/residents', label: 'Мешканці', icon: 'residents' },
  { to: '/admin/announcements', label: 'Оголошення', icon: 'announcements' },
  { to: '/admin/maintenance', label: 'Заявки', icon: 'maintenance' },
  { to: '/admin/payments', label: 'Платежі', icon: 'payments' },
  { to: '/admin/audit-log', label: 'Аудит', icon: 'audit' },
  { to: '/admin/incidents', label: 'Інциденти', icon: 'incidents' },
  { to: '/admin/contacts', label: 'Контакти', icon: 'contacts' },
];
    ...
    <div className="min-h-screen pb-28 lg:pb-0">
      ...
      <main className="px-4 py-4 lg:ml-80 lg:px-8">
        <div className="mb-4 flex flex-wrap items-center justify-between gap-3 lg:hidden">
          <div><p className="font-semibold">{user.email}</p><p className="text-xs text-sky-100/70">Адміністратор · {new Date(user.lastLoginTime).toLocaleString()}</p></div>
