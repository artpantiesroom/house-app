...
export default function MaintenanceAdmin() {
  ...
  return (
    <section className="space-y-5">
      <h1 className="text-3xl font-bold">Заявки на обслуговування</h1>
          ...
          return (
            <article key={request.id} className="glass rounded-2xl p-4">
              <div className="flex flex-wrap items-start justify-between gap-3">
                <div><h2 className="text-lg font-semibold">{request.title}</h2><p className="text-sm text-sky-100/70">{resident?.name || 'Невідомий мешканець'} · {request.category} <DataClassificationBadge level="Internal" /></p><p className="mt-2 text-sm text-sky-100/75">{request.details}</p></div>
                <StatusBadge status={request.status} />
              </div>
              <div className="mt-4 flex flex-wrap gap-2">
                {statuses.map((status) => <button key={status} disabled={savingId === request.id || status === request.status} onClick={() => changeStatus(request, status)} className="focus-ring rounded-xl border border-sky-100/20 px-3 py-2 text-sm disabled:opacity-50">{savingId === request.id && status !== request.status ? 'Збереження...' : { Open: 'Відкрито', 'In Progress': 'У роботі', Resolved: 'Вирішено', Rejected: 'Відхилено' }[status]}</button>)}
              </div>
            </article>
          );
        })}
